/*
    ...
    [解析]
    方法 1: 
        以窗口在数组上滑动，每次都重新计算窗口中最大值，那么总的时间复杂度为 O(n*size)，当窗口大小 size 很大时，不可取。
    方法 2：
        以空间换时间，使用一个双端队列，使得每次获取最大值的操作可以在 O(1) 的时间内完成。
        队列的前边存储当前最大值在数组中对应的下标，而其后存储的是有可能成为新的最大值的候选。
        每次循环需要做两件事情：（1）检查当前最大值是否过期，若过期则弹出最前面的元素；
        （2）更新最大值的候选，从后往前检查将队列中小于当前值的所有元素弹出；
        完成以上两个操作以后将当前元素插入到队列的最后成为候选。
*/
